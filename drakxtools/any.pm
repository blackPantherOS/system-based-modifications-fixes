...
sub setupBootloaderBefore {
	...
	if (my ($biggest_swap) = sort { $b->{size} <=> $a->{size} } grep { isSwap($_) } @$fstab) {
	    ...
	    output("$::prefix/etc/dracut.conf.d/51-blackPanther-resume.conf", qq(add_device+="$biggest_swap_dev"\n));
	}
    }
...
when your installation is complete and you restart your system.", N("blackPantherOS"))),
